    ...
    const { error } = await supabase.from('lockers_overrides').upsert(
      {
        ...
        conflict_ack: input.conflictAck ?? false,
        note: input.note,
...
/**
 * Подтвердить, что бронь и пришедшая АП — это одна и та же кампания.
 * После этого конфликт перестаёт подсвечиваться.
 */
export async function acknowledgeConflict(input: {
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .update({ conflict_ack: true, updated_at: new Date().toISOString() })
      .match({
        club_id: input.clubId,
        month: input.month,
        slot: input.slot,
