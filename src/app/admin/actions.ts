...
/* ------------------------------------------------------------------ */
/* Подсказки о демонтаже                                               */
/* ------------------------------------------------------------------ */

/**
 * Решение по подсказке: запланировать демонтаж, закрыть подсказку или
 * вернуть её обратно в список нерешённых.
 */
export async function setDismount(input: {
  clubId: string;
  clubName: string;
  campaignId: string;
  campaignLabel: string;
  surface: 'lockers' | 'mirrors';
  month: string;
  due: string | null;
  state: 'planned' | 'dismissed' | 'new';
  ...
  try {
    ...
    if (input.state === 'new') {
      // вернуть подсказку в нерешённые — просто убрать решение
      const { error } = await supabase.from('lockers_dismount').delete().match(key);
      if (error) return { ok: false, error: error.message };
    } else {
      const { error } = await supabase.from('lockers_dismount').upsert(
        {
          ...key,
          state: input.state,
          due: input.due,
        ...
        { onConflict: 'club_id,campaign_id,surface_type,month' },
      );
      if (error) return { ok: false, error: error.message };
    }
