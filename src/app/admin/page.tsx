...
import { IconHistory, IconSliders, IconUsers } from '@/components/Icons';

export const dynamic = 'force-dynamic';

const ACTION_LABEL: Record<string, string> = {
  slot_status: 'Изменён статус слота',
  slot_bulk: 'Массовое изменение слотов',
  placement_updated: 'Правка размещения',
...
export default async function AdminOverview() {
  ...
  const [
    ...
    supabase.from('lockers_slot_status').select('id', { count: 'exact', head: true }).eq('month', month),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <StatCard label="Свободных слотов" value={num(s.cells_free)} accent="green" />
        <StatCard label="Занято" value={num(s.cells_occupied)} accent="amber" />
        <StatCard label="Бронь" value={num(s.cells_booked)} accent="brand" hint={`закрыто: ${s.cells_closed}`} />
      ...
      <div className="rounded-xl border border-ink-200/70 bg-ink-50/50 px-4 py-3 text-sm dark:border-white/10 dark:bg-white/5">
        Заполненных статусов в этом месяце: <span className="font-semibold">{num(manualCount ?? 0)}</span>.
        Свободные слоты не хранятся — это состояние по умолчанию.
      </div>
