...
export default async function DashboardHome() {
  ...
  const [{ data: summary }, { data: managers }, { data: clubRows }, session] = await Promise.all([
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <SectionCard
              ...
              description="Учёт по слотам: свободно, бронь, занято, закрыто"
              icon={<IconLayers className="h-5 w-5" />}
              stats={
                s
                  ? [
                      { label: 'Свободно', value: num(s.cells_free) },
                      { label: 'Занято', value: num(s.cells_occupied) },
                      { label: 'Бронь', value: num(s.cells_booked) },
                    ]
                  : []
              }
            />
            <SectionCard
              href="/mirrors"
              title="Зеркала в раздевалках"
              description="Слотов нет — статус ставится сразу на раздевалку"
              icon={<IconLayers className="h-5 w-5" />}
            />
            <SectionCard
              href="/overlaps"
              title="Пересечения между РК"
              description="Клубы, где одновременно идут разные РК по стикерам, зеркалам и экранам"
            ...
            {((managers ?? []) as Manager[]).map((m) => (
              ...
              >
                ...
                <p className="mt-1 flex-1 text-sm text-ink-500 dark:text-ink-400">
                  Кампании, размещения и фотоотчёты
                </p>
          ...
          >
            ...
            <span className="flex-1">
              ...
              <span className="block text-xs text-ink-500 dark:text-ink-400">
                Импорт данных, пользователи и роли, журнал изменений
              </span>
            </span>
