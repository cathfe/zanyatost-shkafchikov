...
export function ManagerDashboard({
  ...
  const totals = useMemo(() => {
    ...
    let placements = 0;
    // задачи подрядчикам: нужны только там, где едет не сам менеджер
    let taskNeeded = 0;
    let taskSet = 0;
    for (const c of campaigns) {
      ...
      for (const p of c.placements) {
        if (!needsTask(p.visit_type)) continue;
        taskNeeded += 1;
        if (p.tasks_done) taskSet += 1;
      }
    }
    return { st, ph, placements, taskNeeded, taskSet, taskMissing: taskNeeded - taskSet };
  }, [campaigns]);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Задачи подрядчикам — второй контролируемый шаг */}
      <section className="card p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold">Задачи монтажникам и БТЛ</h2>
            <p className="text-xs text-ink-500 dark:text-ink-400">
              Где едет не сам менеджер — задача подрядчику должна быть поставлена
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center gap-2 rounded-lg border border-ink-200/70 px-3 py-2 dark:border-white/10">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-500" />
              <span className="text-xs text-ink-500 dark:text-ink-400">Поставлена</span>
              <span className="text-base font-semibold tabular-nums">{totals.taskSet}</span>
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-ink-200/70 px-3 py-2 dark:border-white/10">
              <span className="h-2.5 w-2.5 rounded-full bg-rose-500" />
              <span className="text-xs text-ink-500 dark:text-ink-400">Не поставлена</span>
              <span className="text-base font-semibold tabular-nums">{totals.taskMissing}</span>
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-ink-200/70 px-3 py-2 dark:border-white/10">
              <span className="h-2.5 w-2.5 rounded-full bg-ink-300" />
              <span className="text-xs text-ink-500 dark:text-ink-400">Едет сам</span>
              <span className="text-base font-semibold tabular-nums">
                {totals.placements - totals.taskNeeded}
              </span>
            </div>
          </div>
        </div>

        {totals.taskMissing > 0 && (
          <div className="mt-3 flex items-start gap-2 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-800 dark:bg-rose-500/10 dark:text-rose-200">
            <IconAlert className="mt-0.5 h-4 w-4 shrink-0" />
            По {totals.taskMissing} размещениям подрядчик выбран, но задача ему ещё не поставлена.
          </div>
        )}
      </section>
        ...
        <table className="w-full min-w-[880px] border-collapse">
          <thead className="bg-ink-50/60 dark:bg-white/5">
            <tr>
              ...
              <th className="th text-center">Задача подрядчику</th>
            </tr>
          </thead>
            ...
            {campaign.placements.map((p) => (
              <tr key={p.id} className="align-top hover:bg-ink-50/60 dark:hover:bg-white/5">
                  ...
                  {canEdit ? (
                    <div className="space-y-1.5">
                      <select
                        ...
                        onChange={(e) => {
                          const visit = e.target.value as VisitType;
                          // едет сам менеджер — задачу ставить некому, отметку снимаем
                          onPatch(p, needsTask(visit) ? { visit_type: visit } : { visit_type: visit, tasks_done: false });
                        }}
                ...
                <td className="td text-center">
                  <TaskCell placement={p} canEdit={canEdit} busy={busyId === p.id} onPatch={onPatch} />
                </td>
              </tr>
            ))}
          </tbody>
 ...
 * Задача подрядчику.
 *
 * Отметка означает не «работы выполнены», а «задача поставлена монтажнику
 * или БТЛ». Если едет сам менеджер, ставить задачу некому — ячейка пустая.
 */
function TaskCell({
  placement: p,
  canEdit,
  ...
  if (!needsTask(p.visit_type)) {
    return (
      <span className="text-[11px] text-ink-400" title="Едет сам менеджер — задачу ставить некому">
        не требуется
      </span>
    );
  }
  ...
  return (
    <label className="inline-flex cursor-pointer flex-col items-center gap-1">
      ...
      <span
        className={`text-[11px] ${
          p.tasks_done ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-600 dark:text-rose-300'
        }`}
      >
        {p.tasks_done ? 'поставлена' : 'не поставлена'}
      </span>
    </label>
  );
}
