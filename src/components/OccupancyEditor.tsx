...
export function OccupancyEditor({
  ...
  }, [clubs, query, onlyManual]);

  const go = (next: Record<string, string>) => {
    const sp = new URLSearchParams({ month, surface, ...next });
    startTransition(() => router.push(`/admin/occupancy?${sp.toString()}`, { scroll: false }));
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Занятость</h1>
          <p className="mt-0.5 text-sm text-ink-500 dark:text-ink-400">
            {monthLabel(month)} · {SURFACE_LABEL[surface]} · нажмите на ячейку, чтобы изменить
          </p>
        </div>
                ...
                <tr key={club.club_id} className="hover:bg-ink-50/60 dark:hover:bg-white/5">
                  ...
                  {SLOTS.map((slot) =>
                    (['male', 'female'] as Gender[]).map((g) => {
                      ...
                      const tone = cell ? toneFor(cell.status, cell.free, cell.total) : 'grey';
                      return (
                        <td key={`${slot}${g}`} className="td w-[104px]">
                          <button
                            ...
                            className={`w-full rounded-lg px-2 py-1.5 text-center ring-1 ring-inset transition-transform hover:scale-[1.03] ${TONE_CLASS[tone]}`}
                          >
