...
import { TONE_CLASS, TONE_DOT, monthLabel, monthOptions, num, pct, toneFor } from '@/lib/format';
...
export function OccupancyExplorer({
  ...
  const networks = useMemo(
    () =>
      [...new Set(clubs.map((c) => c.network).filter(Boolean) as string[])].sort((a, b) =>
  ...
  }, [clubs, query, manager, network, status, onlyWithData]);
    ...
    () =>
      filtered.reduce(
        (acc, c) => {
          ...
          acc.closed += c.closedCells;
          return acc;
        },
        { total: 0, free: 0, occupied: 0, reserved: 0, closed: 0 },
      ),
    [filtered],
  );
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Выбор менеджера — основной способ сузить список */}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <StatCard label="Закрытых раздевалок" value={num(totals.closed)} accent={totals.closed ? 'red' : 'grey'} />
      </div>

      <div className="card p-3">
        <div className="grid gap-3 md:grid-cols-[minmax(0,1.6fr)_repeat(2,minmax(0,1fr))_auto]">
          <div className="relative">
            <IconSearch className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-400" />
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Поиск по клубу или сети…"
    ...
    <div className="card grid place-items-center px-6 py-16 text-center">
      <div className="max-w-sm">
        ...
        <p className="mt-1 text-sm text-ink-500 dark:text-ink-400">
          {onlyWithData
            ? 'Снимите фильтры или галочку «скрывать клубы без данных» — по части клубов занятость ещё не загружена.'
...
/**
 * Ячейка занятости. Закрытая раздевалка заливается целиком и подписывается
 * словом ЗАКРЫТ — маленькой иконки для этого недостаточно.
 */
export function CellBox({ cell, size = 'md' }: { cell?: Cell; size?: 'md' | 'lg' }) {
  if (!cell || (cell.total === 0 && cell.status !== 'closed')) {
    return (
      <div className="rounded-lg bg-ink-50 px-2 py-1.5 text-center text-xs text-ink-400 dark:bg-white/5">
        нет данных
      </div>
    );
  }
  ...
  if (cell.status === 'closed') {
    return (
      <div
        className={`rounded-lg px-2 ${size === 'lg' ? 'py-3' : 'py-2'} text-center ring-1 ring-inset ${TONE_CLASS.closed}`}
        title={cell.note ? `Закрыто · ${cell.note}` : 'Закрыто'}
      >
        <span className={`block font-bold uppercase tracking-wide ${size === 'lg' ? 'text-sm' : 'text-[11px]'}`}>
          ● Закрыт
        </span>
        {cell.note && size === 'lg' && (
          <span className="mt-0.5 block text-xs font-normal opacity-90">{cell.note}</span>
        )}
      </div>
    );
  }
  ...
  const title = [
    `Свободно ${cell.free} из ${cell.total}`,
    cell.reserved ? `бронь ${cell.reserved}` : null,
    cell.campaign_label ? `проект: ${cell.campaign_label}` : null,
    cell.note,
  ]
    .filter(Boolean)
    .join(' · ');
  ...
  return (
    <div
      className={`rounded-lg px-2 ${size === 'lg' ? 'py-3' : 'py-1.5'} text-center ring-1 ring-inset ${TONE_CLASS[tone]}`}
      title={title}
    >
      <span className={`font-semibold tabular-nums ${size === 'lg' ? 'text-lg' : 'text-sm'}`}>{cell.free}</span>
      <span className="text-xs opacity-70"> / {cell.total}</span>
      {cell.campaign_label && (
        <span className="mt-0.5 block truncate text-[10px] opacity-80">{cell.campaign_label}</span>
      )}
      {cell.manual && (
        ...
        <table className="w-full min-w-[760px] border-collapse">
          ...
          <tbody className="divide-y divide-ink-200/60 dark:divide-white/5">
            {clubs.map((club) => (
              <tr
                key={club.club_id}
                className={`group hover:bg-ink-50/70 dark:hover:bg-white/5 ${
                  club.closedCells === 4 ? 'bg-rose-50/40 dark:bg-rose-500/5' : ''
                }`}
              >
                ...
                {SLOTS.map((slot) =>
                  (['male', 'female'] as Gender[]).map((g) => (
                    <td key={`${slot}${g}`} className="td w-[110px]">
...
function OccupancyCards({
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {clubs.map((club) => {
        const allClosed = club.closedCells === 4;
        const loadTone = allClosed ? 'closed' : toneFor('available', club.free, club.total || 1);
        return (
          <Link
            key={club.club_id}
            href={`/lockers/${club.club_id}?month=${month}&surface=${surface}`}
            className={`card block p-4 transition-shadow hover:shadow-pop ${
              allClosed ? 'border-rose-300 bg-rose-50/50 dark:border-rose-500/30 dark:bg-rose-500/5' : ''
            }`}
          >
