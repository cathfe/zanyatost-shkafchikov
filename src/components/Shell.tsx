...
import { IconGrid, IconHistory, IconLayers, IconLock, IconLogout, IconSliders, IconUsers } from './Icons';

type NavItem = { href: string; label: string; icon: React.ReactNode };

const PUBLIC_NAV: NavItem[] = [
  { href: '/', label: 'Дашборд', icon: <IconGrid /> },
  { href: '/lockers', label: 'Шкафчики', icon: <IconLayers /> },
  { href: '/mirrors', label: 'Зеркала', icon: <IconLayers /> },
  { href: '/overlaps', label: 'Пересечения между РК', icon: <IconSliders /> },
];

// Раздел «Клубы» намеренно убран из меню: он остаётся живым по прямой ссылке
// для импорта и связей, но в повседневной работе не нужен.
      ...
      <header className="sticky top-0 z-30 border-b border-ink-200/70 bg-white/85 backdrop-blur dark:border-white/10 dark:bg-ink-950/85">
        <div className="mx-auto flex h-14 max-w-[1600px] items-center gap-3 px-4 sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            {role && (
              <span className="hidden text-xs text-ink-500 xl:block dark:text-ink-400">
                {userEmail} · {ROLE_LABEL[role]}
              </span>
            )}
            ...
            {role ? (
              <button onClick={signOut} className="btn-ghost" title="Выйти">
                <IconLogout />
            ...
            <button className="btn-ghost lg:hidden" onClick={() => setOpen((v) => !v)} aria-label="Меню">
