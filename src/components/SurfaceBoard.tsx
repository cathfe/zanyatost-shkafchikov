...
export function SurfaceBoard({
  ...
  const filtered = useMemo(() => {
    ...
    return clubs.filter((c) => {
      ...
      if (statusFilter !== 'all' && c.counts[statusFilter] === 0) return false;
      if (onlyActive && c.counts.free === Object.values(c.cells).length + countMissing(c, withSlots))
        return false;
      return true;
    });
  ...
  const totals = useMemo(() => {
    ...
    for (const c of filtered) for (const s of SLOT_STATUSES) acc[s] += c.counts[s];
    return acc;
  }, [filtered]);
...
function countMissing(club: ClubSlots, withSlots: boolean): number {
  const expected = withSlots ? 4 : 2;
  return Math.max(expected - Object.keys(club.cells).length, 0);
}
