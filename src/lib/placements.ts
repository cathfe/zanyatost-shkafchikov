...
/**
 * Нужно ли ставить задачу подрядчику.
 *
 * Если едет сам менеджер — задачу никому не ставят, контролировать нечего.
 * Если едет монтажник или БТЛ — задача должна быть поставлена, и это
 * отдельный шаг, который легко забыть.
 */
export function needsTask(visit: VisitType): boolean {
  return visit !== 'сам';
}

export const TASK_LABEL: Record<'not_needed' | 'set' | 'missing', string> = {
  not_needed: 'не требуется',
  set: 'поставлена',
  missing: 'не поставлена',
};
...
export type Placement = {
  ...
  /** задача поставлена подрядчику (монтажнику или БТЛ) */
  tasks_done: boolean;
  note: string | null;
};
