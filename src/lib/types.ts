...
/** Строка из lockers_overlaps(month) */
export type OverlapRow = {
  ...
  manager_name: string | null;
  gender: Gender;
  slot1_status: SlotStatus;
  slot1_campaign: string | null;
  slot1_reason: string | null;
  slot1_source: 'ap' | 'manual';
  slot2_status: SlotStatus;
  slot2_campaign: string | null;
  slot2_reason: string | null;
  slot2_source: 'ap' | 'manual';
  is_overlap: boolean;
};
...
export function slotSide(row: OverlapRow, slot: Slot): SlotSide {
  return slot === 1
    ? {
        ...
        reason: row.slot1_reason,
        source: row.slot1_source,
      }
    : {
        slot,
        status: row.slot2_status,
        campaign: row.slot2_campaign,
        reason: row.slot2_reason,
        source: row.slot2_source,
      };
}
