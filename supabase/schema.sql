...
-- ============================================================================
-- ЧАСТЬ 4. Слот — единица учёта. Количество шкафчиков не считаем.
--          Зеркала без слотов. Демо-кампании менеджерского дашборда.
-- ============================================================================

drop table if exists lockers_capacity cascade;
drop table if exists lockers_occupancy cascade;
drop table if exists lockers_overrides cascade;

create table if not exists lockers_campaigns (
  id           uuid primary key default gen_random_uuid(),
  client       text not null,
  label        text not null,
  manager_id   uuid references lockers_managers(id) on delete set null,
  period_start date,
  period_end   date,
...
create table if not exists lockers_placements (
  id            uuid primary key default gen_random_uuid(),
  campaign_id   uuid not null references lockers_campaigns(id) on delete cascade,
  club_id       uuid not null references lockers_clubs(id) on delete cascade,
  format        text not null,
  status        text not null default 'not_sent'
                check (status in ('not_sent','waiting','approved','declined')),
  status_detail text,
  visit_type    text not null default 'сам' check (visit_type in ('сам','монтажник','бтл')),
  assignee      text,
  photo_status  text not null default 'expected'
                check (photo_status in ('expected','received','overdue')),
  photo_due     date,
  tasks_done    boolean not null default false,
  note          text,
  updated_by    uuid references auth.users(id) on delete set null,
  updated_at    timestamptz not null default now(),
  unique (campaign_id, club_id, format)
);

-- Статус слота (шкафчики) или раздевалки (зеркала).
-- Свободное состояние не хранится — строки нет, значит свободно.
create table if not exists lockers_slot_status (
  id             uuid primary key default gen_random_uuid(),
  club_id        uuid not null references lockers_clubs(id) on delete cascade,
  month          date not null,
  surface_type   text not null check (surface_type in ('lockers','mirrors')),
  gender         text not null check (gender in ('male','female')),
  slot           smallint check (slot in (1,2)),
  status         text not null default 'free'
                 check (status in ('free','booked','occupied','closed')),
  campaign_id    uuid references lockers_campaigns(id) on delete set null,
  campaign_label text,
  reason         text,
  source         text not null default 'manual' check (source in ('ap','manual')),
  ap_import_id   uuid references lockers_imports(id) on delete set null,
  updated_by     uuid references auth.users(id) on delete set null,
  updated_at     timestamptz not null default now(),
  created_at     timestamptz not null default now(),
  constraint lockers_slot_shape check (
    (surface_type = 'lockers' and slot is not null) or
    (surface_type = 'mirrors' and slot is null)
  )
);
create unique index if not exists lockers_slot_status_uniq
  on lockers_slot_status (club_id, month, surface_type, gender, coalesce(slot, 0));

alter table lockers_campaigns   enable row level security;
alter table lockers_placements  enable row level security;
alter table lockers_slot_status enable row level security;

do $$
declare t text;
begin
  foreach t in array array['lockers_campaigns','lockers_placements','lockers_slot_status'] loop
    execute format('drop policy if exists %I on %I', t||'_public_read', t);
    execute format('create policy %I on %I for select using (true)', t||'_public_read', t);
    execute format('drop policy if exists %I on %I', t||'_edit_write', t);
    execute format(
...
-- Актуальные функции интерфейса — в миграции lockers_slot_functions:
--   lockers_slots(month), lockers_mirrors(month),
--   lockers_month_summary(month, surface), lockers_club_overlaps(month),
--   lockers_photo_summary(manager_slug)

-- ============================================================================
-- Часть 5. Формат «Экраны» и пересечения РК между форматами
-- (миграции lockers_add_screens_surface, lockers_drop_slot_overlaps)
-- ============================================================================

-- Экраны: ни слота, ни пола — статус ставится на клуб целиком.
-- Поэтому gender становится nullable, а форма строки задаётся одним check.
alter table lockers_slot_status alter column gender drop not null;
alter table lockers_slot_status drop constraint if exists lockers_slot_status_surface_type_check;
alter table lockers_slot_status add constraint lockers_slot_status_surface_type_check
  check (surface_type in ('lockers','mirrors','screens'));

alter table lockers_slot_status drop constraint if exists lockers_slot_shape;
alter table lockers_slot_status add constraint lockers_slot_shape check (
  (surface_type = 'lockers' and slot is not null and gender is not null) or
  (surface_type = 'mirrors' and slot is null     and gender is not null) or
  (surface_type = 'screens' and slot is null     and gender is null)
);

drop index if exists lockers_slot_status_uniq;
create unique index lockers_slot_status_uniq
  on lockers_slot_status (club_id, month, surface_type, coalesce(gender, '-'), coalesce(slot, 0));

-- Пересечения теперь считаются не между слотами одной раздевалки,
-- а между форматами в клубе: стикеры × зеркала × экраны.
drop function if exists lockers_overlaps(date);

-- lockers_club_overlaps(month) — одна строка на клуб, кампании по каждому формату,
-- is_overlap = задействовано больше одного формата и больше одной кампании.
-- Определение функции — в миграции lockers_add_screens_surface.

-- Индекс по выражениям coalesce(...) не выводится из ON CONFLICT (col, ...),
-- из-за чего сохранение статуса из интерфейса падало с ошибкой
-- «there is no unique or exclusion constraint matching the ON CONFLICT specification».
-- В PG 15+ есть NULLS NOT DISTINCT: обычный индекс по колонкам с той же семантикой.
drop index if exists lockers_slot_status_uniq;
create unique index lockers_slot_status_uniq
  on lockers_slot_status (club_id, month, surface_type, gender, slot)
  nulls not distinct;

-- ============================================================================
-- Часть 6. Подсказки о демонтаже
-- (миграции lockers_dismount_hints, lockers_dismount_candidates_fn)
-- ============================================================================

-- РК заканчивается — стикеры и зеркала освобождаются, их можно снять.
-- Дата окончания берётся из периода кампании (lockers_campaigns.period_end).
--
-- Строки нет         → подсказка активна, решения по ней нет;
-- state = 'planned'  → демонтаж запланирован, виден в таблице пересечений;
-- state = 'dismissed'→ подсказку закрыли, она больше не показывается.
create table if not exists lockers_dismount (
  id           uuid primary key default gen_random_uuid(),
  club_id      uuid not null references lockers_clubs(id) on delete cascade,
  campaign_id  uuid not null references lockers_campaigns(id) on delete cascade,
  surface_type text not null check (surface_type in ('lockers','mirrors')),
  month        date not null,
  state        text not null check (state in ('planned','dismissed')),
  due          date,
  note         text,
  updated_by   uuid references auth.users(id) on delete set null,
  updated_at   timestamptz not null default now(),
...
create unique index if not exists lockers_dismount_uniq
  on lockers_dismount (club_id, campaign_id, surface_type, month);

alter table lockers_dismount enable row level security;
drop policy if exists lockers_dismount_public_read on lockers_dismount;
create policy lockers_dismount_public_read on lockers_dismount for select using (true);
drop policy if exists lockers_dismount_edit_write on lockers_dismount;
create policy lockers_dismount_edit_write on lockers_dismount
  for all to authenticated using (lockers_can_edit()) with check (lockers_can_edit());

-- lockers_dismount_candidates(month) — занятые слоты стикеров и зеркал,
-- у кампании которых period_end приходится на этот месяц.
-- Определение — в миграции lockers_dismount_candidates_fn.
